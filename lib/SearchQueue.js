  ...
  _process: function(snap) {
     ...
     if (this._assertValidSearch(key, dat)) {
      // Perform ElasticSearch query
        ...
        if (typeof dat.sort !== 'undefined') {
           query.sort = dat.sort;
        }
        if (typeof dat.from !== 'undefined') {
           query.from = dat.from;
        }
        if (typeof dat.size !== 'undefined') {
           query.size = dat.size;
        }
        if (typeof dat.min_score !== 'undefined') {
           query.min_score = dat.min_score;
        }
   ...
   _assertValidSearch: function(key, props) {
      var res = true;
      if( typeof(props) !== 'object' || !props.index || !props.query ) {
         this._replyError(key, 'search request must be a valid object with keys index and query');
      }
      return res;
   },
